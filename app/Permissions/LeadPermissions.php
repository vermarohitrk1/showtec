    ...
    public function check($action = '', $lead = '') {
        ...
        if ($action == 'users') {
            ...
            foreach ($users as $user) {
                if ($user->id > 0) {
                    //global user
                    if ($user->roles->first()->role_leads >= 1 && $user->roles->first()->role_leads_scope == 'global') {
                        $list[] = $user->id;
                        continue;
                    }
                    //assigned
        ...
        if ($action == 'view') {
            //global user
            if (auth()->user()->roles->first()->role_leads >= 1 && auth()->user()->roles->first()->role_leads_scope == 'global') {
                return true;
            }
            //assigned
        ...
        if ($action == 'edit') {
            //global user
            if (auth()->user()->roles->first()->role_leads >= 2 && auth()->user()->roles->first()->role_leads_scope == 'global') {
                return true;
            }
            //assigned
            if ($assigned_users->contains(auth()->id())) {
        ...
        if ($action == 'participate') {
            ...
            if (auth()->user()->roles->first()->role_leads_scope == 'global' && auth()->user()->roles->first()->role_leads >= 2) {
                return true;
            }
        }
        ...
        if ($action == 'delete') {
            //global user
            if (auth()->user()->roles->first()->role_leads >= 3 && auth()->user()->roles->first()->role_leads_scope == 'global') {
                return true;
            }
            //creator
            if ($lead->lead_creatorid == auth()->id()) {
