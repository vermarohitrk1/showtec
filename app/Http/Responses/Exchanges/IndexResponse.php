    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/exchanges/components/table/ajax';
                    $dom_container = '#exchanges-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/exchanges/components/table/table';
                    $dom_container = '#exchanges-table-wrapper';
                    $dom_action = 'replace-with';
                    break;

                default:
                    //template and dom - for ajax initial loading
                    $template = 'pages/exchanges/wrapper';
            ...
            if ($exchanges->currentPage() < $exchanges->lastPage()) {
                $url = loadMoreButtonUrl($exchanges->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'exchanges'))->render();
            $jsondata['dom_html'][] = array(
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.returns')),
            ];
            ...
            //dd('normal view');
            //standard view
            $page['url'] = loadMoreButtonUrl($exchanges->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vendors-td-container';
            $page['visibility_show_load_more'] = ($exchanges->currentPage() < $exchanges->lastPage()) ? true : false;
            return view('pages/exchanges/wrapper', compact('page', 'exchanges'))->render();
        }
    }
}
