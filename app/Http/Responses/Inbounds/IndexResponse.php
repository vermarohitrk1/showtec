    ...
    public function toResponse($request)
    {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            switch (request('action')) {
                ...
                case 'load':
                    $template = 'pages/inbounds/components/table/ajax';
                    $dom_container = '#inbounds-td-container';
                    $dom_action = 'append';
                    break;
                ...
                case 'search':
                    $template = 'pages/inbounds/components/table/table';
                    $dom_container = '#inbounds-table-wrapper';
                    $dom_action = 'replace-with';
                    break;

                default:
                    //template and dom - for ajax initial loading
                    $template = 'pages/inbounds/wrapper';
            ...
            if ($bounds->currentPage() < $bounds->lastPage()) {
                $url = loadMoreButtonUrl($bounds->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'bounds'))->render();
            $jsondata['dom_html'][] = array(
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.inbound_outbound')),
            ];
            ...
            //dd('normal view');
            //standard view
            $page['url'] = loadMoreButtonUrl($bounds->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vendors-td-container';
            $page['visibility_show_load_more'] = ($bounds->currentPage() < $bounds->lastPage()) ? true : false;
            return view('pages/inbounds/wrapper', compact('page', 'bounds'))->render();
        }
    }
}
