...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/vendors/components/table/ajax';
                $dom_container = '#vendors-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/vendors/components/table/table';
                $dom_container = '#vendors-table-wrapper';
                $dom_action = 'replace-with';
                break;

            default:
                //template and dom - for ajax initial loading
                $template = 'pages/vendors/tabswrapper';
            ...
            if ($vendors->currentPage() < $vendors->lastPage()) {
                $url = loadMoreButtonUrl($vendors->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'stats', 'vendors'))->render();
            $jsondata['dom_html'][] = array(
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($vendors->currentPage() + 1, request('source'));
            $page['loading_target'] = 'vendors-td-container';
            $page['visibility_show_load_more'] = ($vendors->currentPage() < $vendors->lastPage()) ? true : false;
            return view('pages/vendors/wrapper', compact('page', 'stats', 'vendors', 'categories', 'tags'))->render();
        }
