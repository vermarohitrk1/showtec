...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the purchase orders
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/purchaseorders/components/table/ajax';
                $dom_container = '#purchase-orders-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/purchaseorders/components/table/table';
                $dom_container = '#purchase-orders-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/purchaseorders/tabswrapper';
            ...
            if ($purchase_orders->currentPage() < $purchase_orders->lastPage()) {
                $url = loadMoreButtonUrl($purchase_orders->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'purchase_orders', 'categories', 'tags'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.purchase_orders')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($purchase_orders->currentPage() + 1, request('source'));
            $page['loading_target'] = 'temp-td-container';
            $page['visibility_show_load_more'] = ($purchase_orders->currentPage() < $purchase_orders->lastPage()) ? true : false;
            return view('pages/purchaseorders/wrapper', compact('page', 'purchase_orders', 'categories', 'tags'))->render();
        }
