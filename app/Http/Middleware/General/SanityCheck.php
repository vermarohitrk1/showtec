...
class SanityCheck {
    ...
    public function handle($request, Closure $next) {
        ...
        // dd(env('SETUP_STATUS'));
        // if (env('SETUP_STATUS') == 'NOTCOMPLETED') {
    ...
    private function validateSettingsFile() {
        ...
        foreach ($task_statuses as $key) {
            if (!array_key_exists($key, config('settings.task_statuses'))) {
                Log::critical('/config/settings.php file error: (invalid $task_statuses array)', ['process' => '[System Sanity Check]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
                abort(409, __('lang.application_error'));
            }
        }
