...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_team) {
            if (auth()->user()->roles->first()->role_leads >= 1) {
                //[limit] - for users with only local level scope
                if (auth()->user()->roles->first()->role_leads_scope == 'own') {
    ...
    private function fronteEnd() {
        ...
        //permissions -adding
        if (auth()->user()->roles->first()->role_leads >= 2) {
            config([
                //visibility
        ...
        if (auth()->user()->roles->first()->role_leads >= 3) {
            config([
                //visibility
        ...
        if (auth()->user()->is_team) {
            if (auth()->user()->roles->first()->role_leads_scope == 'global') {
                config([
                    //visibility
    ...
    function toggleKanbanView() {
        ...
        if (request('toggle') == 'layout') {
            //toggle database settings
            auth()->user()->settings->pref_view_leads_layout = (auth()->user()->settings->pref_view_leads_layout == 'kanban') ? 'list' : 'kanban';
            auth()->user()->save();
        }

        //css setting for body
        if (auth()->user()->settings->pref_view_leads_layout == 'kanban') {
            config([
