...
class DestroyRepository
{
    ...
    public function destroyEmployee($employee_id)
    {
        ...
        if (!$employee = \App\Models\User::Where('id', $employee_id)->first()) {
        ...
        //delete queued emails
        // \App\Models\EmailQueue::Where('emailqueue_resourcetype', 'client')->Where('emailqueue_resourceid', $employee->id)->delete();
    ...
    /**
     * destroy a purchase order and all related items
     * @param numeric $purchase_order_id
     * @return bool or id of record
     */
    public function destroyPurchaseOrder($purchase_order_id)
    {

        //validate purchase order
        if (!is_numeric($purchase_order_id)) {
            Log::error("validation error - invalid params", ['process' => '[destroy][purchase_order]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }

        //get purchase order and validate
        if (!$purchase_order = \App\Models\PurchaseOrder::Where('po_invoiceid', $purchase_order_id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][purchase_order]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        ...
        //delete line items
        $purchase_order->lineitems()->delete();
        ...
        //delete events
        \App\Models\Event::Where('event_parent_type', 'purchaseorder')->where('event_parent_id', $purchase_order_id)->delete();
        \App\Models\EventTracking::Where('parent_type', 'purchaseorder')->where('parent_id', $purchase_order_id)->delete();

        //delete queued emails
        \App\Models\EmailQueue::Where('emailqueue_resourcetype', 'purchaseorder')->Where('emailqueue_resourceid', $purchase_order_id)->delete();

        //delete invoice
        $purchase_order->delete();
    }

    public function destroyVendorFile($id)
    {
        if (!$vendorFile = \App\Models\VendorFile::Where('fileid', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $vendorFile->delete();
    ...
    public function destroyFixedAsset($id)
    {
        if (!$fixedAsset = \App\Models\FixedAsset::Where('assetid', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $fixedAsset->delete();
    ...
    {
        if (!$return = \App\Models\Exchange::Where('id', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $return->delete();
    ...
    {
        if (!$bound = \App\Models\Inbound::Where('id', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $bound->delete();
    ...
    {
        if (!$warehouse = \App\Models\Warehouse::Where('warehouse_id', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $warehouse->delete();
        \App\Models\Rack::where('rack_warehouseid', $id)->delete();
    }

    public function destroyRack($id)
    {
        if (!$rack = \App\Models\Rack::Where('rack_id', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $rack->delete();
    ...
    {
        if (!$stock = \App\Models\Stock::Where('id', $id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][subscription]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        $stock->delete();
