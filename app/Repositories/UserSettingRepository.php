...
class UserSettingRepository {
    ...
    protected $users;
    /**
     * The users setting repository instance.
     */
    protected $user_setting;
    /**
     * Inject dependecies
     */
    public function __construct(User $users, UserSetting $user_setting) {
        $this->users = $users;
    ...
    public function search($id = '') {
        ...
        //user object
        $users = $this->user_setting->newQuery();
    ...
    public function create($emp_id) {
        ...
        $user = new $this->user_setting;
        $user->user_id = $emp_id;
        $user->theme = 'default';
        $user->pref_language = 'english';
        $user->pref_email_notifications = 'yes';
        $user->pref_leftmenu_position = 'collapsed';
        $user->pref_statspanel_position = 'collapsed';
        $user->pref_filter_own_tasks = 'no';
        $user->pref_filter_own_projects = 'no';
        $user->pref_filter_show_archived_projects = 'no';
        $user->pref_filter_show_archived_tasks = 'no';
        $user->pref_filter_show_archived_leads = 'no';
        $user->pref_filter_own_leads = 'no';
        $user->pref_view_tasks_layout = 'kanban';
        $user->pref_view_leads_layout = 'kanban';
        $user->welcome_email_sent = 'no';
        if ($user->save()) {
            
                return $user;
        } else {
            Log::error("record could not be saved - database error", ['process' => '[UserSettingRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
