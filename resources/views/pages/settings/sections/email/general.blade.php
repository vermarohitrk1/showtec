...
<form class="form" id="settingsFormEmailGeneral">
    ...
    <div class="form-group row">
        ...
        <div class="col-12">
            <input type="text" class="form-control form-control-sm" id="settings_email_from_address"
                name="settings_email_from_address" value="{{ $settings->get_key('settings_email_from_address') ?? '' }}">
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-12">
            <input type="text" class="form-control form-control-sm" id="settings_email_from_name"
                name="settings_email_from_name" value="{{ $settings->get_key('settings_email_from_name') ?? '' }}">
        </div>
    </div>
        ...
        <div class="col-12">
            <select class="select2-basic form-control form-control-sm" id="settings_email_server_type"
                ...
                <option value="sendmail"
                    {{ runtimePreselected('sendmail', $settings->get_key('settings_email_server_type') ?? '') }}>
                    Sendmail</option>
                <option value="smtp" {{ runtimePreselected('smtp', $settings->get_key('settings_email_server_type') ?? '') }}>
                    SMTP</option>
            </select>
        </div>
    </div>
    ...
    <div class="splash-text">
        <div class="alert alert-danger">{{ cleanLang(__('lang.cronjob_and_emails')) }}. <a href="https://growcrm.io/documentation/cron-job-settings/" target="_blank">@lang('lang.more_information')</a></div>
    </div>
    @endif
